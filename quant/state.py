...
from quant.indicators import ATR, EMA, MACD, RSI, SMA, BollingerBands
from quant.strategies import BreakoutStrategy, MeanReversionStrategy, MomentumStrategy
...
class State(rx.State):
    ...
    def set_ticker(self, new_ticker: str) -> None:
    ...
    def fetch_data(self) -> None:
        ...
        try:
            ...
            if "Date" in polars_df.columns:
                dict_data = polars_df.with_columns(
    ...
    def run_backtest(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Running backtest for {self.ticker} with {self.selected_strategy} strategy"
            )
            db = Database()
    ...
    def set_new_position_ticker(self, ticker: str) -> None:
        """Set the new position ticker."""
        self.new_position_ticker = ticker.upper()

    def set_new_position_quantity(self, quantity: str) -> None:
        """Set the new position quantity."""
        self.new_position_quantity = quantity

    def set_new_position_price(self, price: str) -> None:
        """Set the new position price."""
        self.new_position_price = price

    def add_portfolio_position(self) -> None:
        """Add a new position to the portfolio."""
        if (
            not self.new_position_ticker
            or not self.new_position_quantity
            or not self.new_position_price
        ):
            return
    ...
    def _update_portfolio_metrics(self) -> None:
        ...
        self.portfolio_allocation_data = [
            {"ticker": h["ticker"], "value": h["market_value"]} for h in self.portfolio_holdings
        ]

    def optimize_portfolio_sharpe(self) -> None:
        """Optimize portfolio for maximum Sharpe ratio."""
        # Implementation would use portfolio optimization module

    def optimize_portfolio_volatility(self) -> None:
        """Optimize portfolio for minimum volatility."""

    def optimize_portfolio_risk_parity(self) -> None:
        """Optimize portfolio using risk parity."""

    def apply_rebalancing(self) -> None:
        """Apply the optimized rebalancing."""

    # Risk analytics methods
    def set_var_confidence_level(self, level: str) -> None:
        """Set VaR confidence level."""
        self.var_confidence_level = level

    def set_var_method(self, method: str) -> None:
        """Set VaR calculation method."""
        self.var_method = method

    def set_var_time_horizon(self, horizon: str) -> None:
        """Set VaR time horizon."""
        self.var_time_horizon = horizon

    def calculate_portfolio_var(self) -> None:
    ...
    def load_portfolio_history(self) -> None:
        """Load portfolio history for drawdown analysis."""

    def set_max_position_size(self, size: str) -> None:
        """Set max position size."""
        self.max_position_size = size

    def set_max_drawdown_alert(self, alert: str) -> None:
        """Set max drawdown alert."""
        self.max_drawdown_alert = alert

    def set_max_daily_loss(self, loss: str) -> None:
        """Set max daily loss."""
        self.max_daily_loss = loss

    def set_min_sharpe_ratio(self, ratio: str) -> None:
        """Set min Sharpe ratio."""
        self.min_sharpe_ratio = ratio

    def save_risk_limits(self) -> None:
        """Save risk limits."""

    # Strategy builder methods
    def set_custom_strategy_name(self, name: str) -> None:
        """Set custom strategy name."""
        self.custom_strategy_name = name

    def set_custom_strategy_type(self, strategy_type: str) -> None:
        """Set custom strategy type."""
        self.custom_strategy_type = strategy_type

    def toggle_indicator_sma(self, checked: bool) -> None:
        """Toggle SMA indicator."""
        self.indicator_sma = checked

    def toggle_indicator_ema(self, checked: bool) -> None:
        """Toggle EMA indicator."""
        self.indicator_ema = checked

    def toggle_indicator_macd(self, checked: bool) -> None:
        """Toggle MACD indicator."""
        self.indicator_macd = checked

    def toggle_indicator_adx(self, checked: bool) -> None:
        """Toggle ADX indicator."""
        self.indicator_adx = checked

    def toggle_indicator_rsi(self, checked: bool) -> None:
        """Toggle RSI indicator."""
        self.indicator_rsi = checked

    def toggle_indicator_stochastic(self, checked: bool) -> None:
        """Toggle Stochastic indicator."""
        self.indicator_stochastic = checked

    def toggle_indicator_roc(self, checked: bool) -> None:
        """Toggle ROC indicator."""
        self.indicator_roc = checked

    def toggle_indicator_williams(self, checked: bool) -> None:
        """Toggle Williams %R indicator."""
        self.indicator_williams = checked

    def toggle_indicator_bollinger(self, checked: bool) -> None:
        """Toggle Bollinger Bands indicator."""
        self.indicator_bollinger = checked

    def toggle_indicator_atr(self, checked: bool) -> None:
        """Toggle ATR indicator."""
        self.indicator_atr = checked

    def toggle_indicator_keltner(self, checked: bool) -> None:
        """Toggle Keltner Channel indicator."""
        self.indicator_keltner = checked

    def toggle_indicator_obv(self, checked: bool) -> None:
        """Toggle OBV indicator."""
        self.indicator_obv = checked

    def toggle_indicator_vwap(self, checked: bool) -> None:
        """Toggle VWAP indicator."""
        self.indicator_vwap = checked

    def toggle_indicator_mfi(self, checked: bool) -> None:
        """Toggle MFI indicator."""
        self.indicator_mfi = checked

    def set_sma_period(self, period: str) -> None:
        """Set SMA period."""
        self.sma_period = period

    def set_ema_period(self, period: str) -> None:
        """Set EMA period."""
        self.ema_period = period

    def set_rsi_period(self, period: str) -> None:
        """Set RSI period."""
        self.rsi_period = period

    def set_bollinger_period(self, period: str) -> None:
        """Set Bollinger Bands period."""
        self.bollinger_period = period

    def set_bollinger_std(self, std: str) -> None:
        """Set Bollinger Bands standard deviation."""
        self.bollinger_std = std

    def set_entry_conditions(self, conditions: str) -> None:
        """Set entry conditions."""
        self.entry_conditions = conditions

    def set_exit_conditions(self, conditions: str) -> None:
        """Set exit conditions."""
        self.exit_conditions = conditions

    def set_position_size_pct(self, pct: str) -> None:
        """Set position size percentage."""
        self.position_size_pct = pct

    def set_stop_loss_pct(self, pct: str) -> None:
        """Set stop loss percentage."""
        self.stop_loss_pct = pct

    def set_take_profit_pct(self, pct: str) -> None:
        """Set take profit percentage."""
        self.take_profit_pct = pct

    def set_max_positions(self, max_pos: str) -> None:
        """Set maximum positions."""
        self.max_positions = max_pos

    def save_custom_strategy(self) -> None:
        ...
        logger.info("Saved strategy: %s", self.custom_strategy_name)

    def test_custom_strategy(self) -> None:
        """Test the custom strategy."""
        logger.info("Testing strategy: %s", self.custom_strategy_name)

    def reset_strategy_builder(self) -> None:
